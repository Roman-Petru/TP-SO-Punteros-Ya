...
static void ciclo_pedido(t_pedido* pedido)
{
	while(true)
	{
		...
		log_info(logger_app, "el pedido %d se movio a (%d,%d)", pedido->id_pedido, pedido_posicion(pedido)->x, pedido_posicion(pedido)->y);
...
t_pedido* crear_pedido(int id, t_posicion* posicion_de_restaurante, t_posicion* posicion_cliente, bool resto_default)
{
	...
	log_info(logger_app, "Se recibió un nuevo pedido:");
	log_info(logger_app, "	Id del pedido: %d", nuevo_pedido->id_pedido);
	log_info(logger_app, "	Ubicación del cliente: (%d, %d)", nuevo_pedido->posicion_cliente->x, nuevo_pedido->posicion_cliente->y);
	log_info(logger_app, "	Ubicación del restaurante: (%d, %d)", nuevo_pedido->posicion_de_restaurante->x, nuevo_pedido->posicion_de_restaurante->y);
...
int pedido_estimacion(t_pedido* pedido)
{
	...
	double valor_alpha = config_get_double_value(config_app, "ALPHA");
...
{
	t_posicion* posicion_de_restaurante = posicion_crear(config_get_int_value(config_app, "POSICION_REST_DEFAULT_X"),config_get_int_value(config_app, "POSICION_REST_DEFAULT_Y"));
